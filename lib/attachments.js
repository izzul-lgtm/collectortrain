...
export const ATTACHMENT_MAX_BYTES = 10 * 1024 * 1024; // 10MB — mesej/discussion
export const ATTACHMENT_TTL_MS = 48 * 60 * 60 * 1000; // 48 jam (mesej/discussion sahaja)
...
// Learning (SOP/onboarding) benarkan fail lebih besar + video terus dimuat
// naik — SENGAJA tak dikongsi dengan had 10MB mesej sebab video mudah >10MB.
// Fail Learning pun tak kena purge 48 jam (lihat schema.sql/route.js Learning).
export const LEARNING_ATTACHMENT_MAX_BYTES = 100 * 1024 * 1024; // 100MB
export const LEARNING_ATTACHMENT_ALLOWED_TYPES = [
...
// learning_steps. opts.maxBytes/opts.allowedTypes override had default
// (10MB, tiada video) — Learning route hantar had yang lagi longgar.
export async function uploadAttachment(file, ownerId, opts = {}) {
  const maxBytes = opts.maxBytes || ATTACHMENT_MAX_BYTES;
  const allowedTypes = opts.allowedTypes || ATTACHMENT_ALLOWED_TYPES;
  if (!file || typeof file.arrayBuffer !== 'function') {
    throw new Error('Fail tidak sah.');
  }
  if (file.size > maxBytes) {
    throw new Error(`Fail terlalu besar. Had maksimum ${Math.round(maxBytes / 1024 / 1024)}MB.`);
  }
