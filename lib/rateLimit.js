 ...
 * @param {Request} request  - Next.js request object (untuk dapat x-session-token)
 ...
 * @param {string}  [opts.key]     - Identity key override. Guna ni untuk route yang
 *                                   belum ada x-session-token (cth: login, sebelum auth) —
 *                                   biasanya gabungan IP + ID yang dicuba.
 * @returns {Response|null}  null = OK, Response = kena reject
 */
export function rateLimit(request, bucket, { max = 10, windowMs = 60_000, key: keyOverride } = {}) {
  const identity = keyOverride || (request.headers.get('x-session-token') || 'anon').trim();
