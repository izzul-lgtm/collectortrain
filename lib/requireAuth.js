...
export async function requireAuth(request, { roles } = {}) {
  ...
  if (!userId) {
    return _deny('Akses ditolak: tiada ID pengguna dalam request.', 401);
  }
  ...
  if (cached && cached.expiresAt > Date.now()) {
    if (roles && !roles.includes(cached.role)) {
      return _deny(`Akses ditolak: perlu role ${roles.join('/')} untuk tindakan ini.`, 403);
    }
    return null; // OK
  }
  ...
  try {
    ...
    if (!data) {
      return _deny('Akses ditolak: pengguna tidak dijumpai atau sesi tidak sah.', 401);
    }
    ...
    return _deny('Ralat semak sesi: ' + err.message, 500);
  }
}
